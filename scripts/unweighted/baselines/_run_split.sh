...
if [[ "$#" -lt 4 ]]; then
  echo "usage: $0 <mcs|inverse_square> <family> <first4|last3> <recovery> [--dry-run] [runner args...]" >&2
  exit 2
fi
...
case "$scheme" in
  mcs|inverse_square) ;;
  *) echo "unsupported unweighted baseline: $scheme" >&2; exit 2 ;;
esac
case "$family" in
...
suite_config="configs/unweighted/baselines/${family}/sunset/${scheme}_suite.json"
case_name="baseline_${scheme}__recover_${recovery}"
result_tag="unweighted/${family}/sunset/${split}_${scheme}_recover_${recovery}"
command=(
  "$PYTHON_BIN"
