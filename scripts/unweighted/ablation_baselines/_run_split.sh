#!/usr/bin/env bash
set -euo pipefail

if [[ "$#" -lt 4 ]]; then
  echo "usage: $0 <mcs|inverse_square> <family> <first4|last3> <cfg1|cfg1p5|cfg3|cfg5> [--dry-run] [runner args...]" >&2
  exit 2
fi

scheme="$1"
family="$2"
split="$3"
cfg="$4"
shift 4

case "$scheme" in
  mcs|inverse_square) ;;
  *) echo "unsupported unweighted ablation baseline: $scheme" >&2; exit 2 ;;
esac
case "$family" in
  prompt_matched|prompt_mismatched|out_of_range) ;;
  *) echo "unsupported unweighted experiment family: $family" >&2; exit 2 ;;
esac
case "$split" in
  first4) sampling_percentages="0.00015625,0.0003125,0.000625,0.00125" ;;
  last3) sampling_percentages="0.0025,0.005,0.01" ;;
  *) echo "unsupported unweighted split: $split" >&2; exit 2 ;;
esac
case "$cfg" in
  cfg1|cfg1p5|cfg3|cfg5) ;;
  *) echo "unsupported recovery CFG: $cfg" >&2; exit 2 ;;
esac

dry_run=false
runner_args=()
for argument in "$@"; do
  if [[ "$argument" == "--dry-run" ]]; then
    dry_run=true
  else
    runner_args+=("$argument")
  fi
done

if [[ -f ./run_conditioning_regression.py && -d ./configs/unweighted ]]; then
  root_dir="."
else
  script_dir="$(cd -P -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
  root_dir="$(cd -P -- "$script_dir/../../.." && pwd -P)"
fi
if [[ ! -f "$root_dir/run_conditioning_regression.py" ]]; then
  echo "could not locate the ActiveConditionalGCS project root" >&2
  exit 2
fi
cd "$root_dir"

if [[ -z "${PYTHON_BIN:-}" ]]; then
  if command -v python >/dev/null 2>&1; then
    PYTHON_BIN="python"
  elif [[ -x "/opt/anaconda3/bin/python" ]]; then
    PYTHON_BIN="/opt/anaconda3/bin/python"
  elif command -v python3.12 >/dev/null 2>&1; then
    PYTHON_BIN="python3.12"
  else
    PYTHON_BIN="python3"
  fi
fi

export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

suite_config="configs/unweighted/ablation/baselines/${family}/sunset/${scheme}_suite.json"
case_name="baseline_${scheme}__recover_prompt_sunset_beach_${cfg}"
result_tag="unweighted/ablation/${family}/sunset/${split}_${scheme}_${cfg}"
command=(
  "$PYTHON_BIN"
  run_conditioning_regression.py
  --suite-config "$suite_config"
  --tag "$result_tag"
  --cases "$case_name"
  --sampling-methods "$scheme"
  --sampling-percentages "$sampling_percentages"
)

if [[ "$dry_run" == true ]]; then
  echo "scheme=$scheme family=$family split=$split cfg=$cfg"
  echo "rates=$sampling_percentages tag=$result_tag"
  "${command[@]}" --list-cases
  exit 0
fi

exec "${command[@]}" "${runner_args[@]}"
