"""Loading, validation, and plotting for the weighted out-of-range experiment."""
...
RUN_ROOT = PROJECT_ROOT / "results" / "weighted" / "out_of_range"
...
def load_rows(*, include_partial: bool = True) -> pd.DataFrame:
    ...
    for sampling_rank, (law, info) in enumerate(LAW_INFO.items()):
        ...
        for recon_rank, recovery_condition in enumerate(RECOVERY_CONDITIONS):
            ...
            if frame.empty:
                continue
            case_tag = f"results/weighted/out_of_range/{law}/{case_name}"
