"""Loading, validation, and plotting for the weighted prompt-matched study."""
...
RUN_ROOT = PROJECT_ROOT / "results" / "weighted" / "prompt_matched"
...
def load_rows(*, include_partial: bool = True) -> pd.DataFrame:
    ...
    for sampling_rank, (law, info) in enumerate(LAW_INFO.items()):
        ...
        for recon_rank, recovery_condition in enumerate(RECOVERY_CONDITIONS):
            ...
            if frame.empty:
                continue
            case_tag = f"results/weighted/prompt_matched/{law}/{case_name}"
